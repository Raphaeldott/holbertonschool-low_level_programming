...
char *create_array(unsigned int size, char c);/* create an array of char of specified size */
char *_strdup(char *str);/* pointer to a copy */
char *str_concat(char *s1, char *s2);/* concatenate 2 string with malloc */
int **alloc_grid(int width, int height);/* return a pointer to a 2 dim array of ints */
void free_grid(int **grid, int height);/* frees a 2 dim grid previously created by alloc_grid */
void *malloc_checked(unsigned int b);/* checker for malloc fails */
char *string_nconcat(char *s1, char *s2, unsigned int n);/* concat n bytes */
void *_calloc(unsigned int nmemb, unsigned int size);/* allocat for n memb of element */
int *array_range(int min, int max);/* array of min to max */
